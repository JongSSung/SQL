...
ON EMP.deptno = 20 OR EMP.deptno =30
ORDER BY HIREDATE DESC

CREATE TABLE TEST_EMP AS SELECT * FROM EMP;
CREATE TABLE TEST_DEPT AS SELECT * FROM DEPT;

ALTER TABLE TEST_EMP ADD GRADE VARCHAR(5);

SELECT * FROM TEST_EMP ORDER BY MGR ASC

UPDATE TEST_EMP SET GRADE=1 WHERE SAL>=1500
UPDATE TEST_EMP SET GRADE=0 WHERE SAL<=1499


UPDATE TEST_EMP SET COMM = 0 WHERE COMM IS NULL;

UPDATE TEST_EMP SET MGR = 0 WHERE MGR IS NULL;